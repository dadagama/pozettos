...
function validarCampoFecha(campoFecha, nombreCampo, admitirVacio)
{
  ...
  if (admitirVacio && campoFecha.val().length == 0)
		return true;
	else if (!admitirVacio && !(campoFecha.val().length > 0))
  { 
		var mensaje = "El campo ["+nombreCampo+"] no puede estar vacío.";
		alert(mensaje);
    return false;
  }  
  ...
  {
		var mensaje = "El campo ["+nombreCampo+"] no tiene un año válido.";
		alert(mensaje);
    return false;
  }
  // Valido el Mes
  ...
  {
		var mensaje = "El campo ["+nombreCampo+"] no tiene un mes válido.";
		alert(mensaje);
    return false;
  }
  // Valido el Dia
  ...
  {
		var mensaje = "El campo ["+nombreCampo+"] no tiene un día válido.";
		alert(mensaje);
    return false;
  }
